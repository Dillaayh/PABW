// pages/profile/statusPesanan/index.js
'use client';
...
export default function StatusPesanan() {
  ...
  return (
    <div className="flex flex-row min-h-screen bg-[#EAF0FF] p-4 lg:p-8 text-black">
      ...
      {/* Status Pesanan */}
      {/* Tambahkan overflow-y-auto dan max-h-screen untuk membuatnya bisa discroll secara independen */}
      <div className="w-full lg:flex-1 bg-white rounded-2xl shadow-md p-6 lg:p-8 lg:ml-8 overflow-y-auto max-h-[calc(100vh-2rem)] lg:max-h-full">
        ...
        {pesanan.length === 0 ? (
          ...
          <div className="space-y-4">
            {pesanan.map((item) => (
              <Link
                key={item.id}
                href={`/pages/profile/statusPesanan/${item.id}`}
                className="block"
              >
